...
`attestation.py` (stdlib `json`/`hashlib`/`hmac`/`uuid`/`datetime`) — a **tamper-evident, point-in-time compliance evidence bundle** for auditors. Detached layout `{"body": {...}, "seal": {...}}`: everything attestable lives in `body`, `seal` authenticates `body`. Wired via `_ReportMixin.save_attestation` (+ `_source_artifact`, `_load_attest_key`); CLI `--attest`/`--attest-key`/`--attest-html`/`--attest-oscal`/`--attest-org`/`--attest-verify` on BOTH scanners; `_attest_verify_action` is the standalone early-exit verify (exit 0 ok / 2 tampered).
- **Reuse, don't fork**: control PASS/FAIL comes from `benchmark_score()`; risk-acceptance from `posture.Exceptions.match()` + `posture.finding_entity()`, keyed on `_posture_host()` — so the attestation and the posture report can never disagree. `build_attestation` excludes INFO to match `benchmark_score`'s reportable basis. Exceptions.match needs a **NAIVE** `now` (it compares against naive parsed dates — a tz-aware now raises); build derives it from the tz-aware report time.
- **Anti-overclaim is a feature**: pass-rate denominator = controls the tool evaluates (N); `coverage_percent_of_framework` emitted ONLY when a real total is in `FRAMEWORK_TOTALS` (sparse by design — an unverified M is worse than none); method = Examine-only; the disclaimer never says "compliant". Fed the UNFILTERED `_all_findings` so a `--severity` display filter can't inflate the pass rate.
- **Tamper-evidence** (the core): `canonical_bytes()` = `json.dumps(sort_keys=True, separators=(",",":"), ensure_ascii=False, allow_nan=False)` — **floats are BANNED** from the body (Python vs ECMAScript float formatting is non-reproducible; EPSS/scores would be strings/ints). `_attestation_records(body)` = every per-control result + every risk-acceptance (id-sorted) — the load-bearing extractor **shared by build and verify**. `build_manifest` = per-record SHA-256 + **RFC 6962 Merkle root** (leaf=`sha256(0x00‖rec)`, node=`sha256(0x01‖L‖R)`, split at the largest power of two STRICTLY < n). `seal_body` = SHA-256 digest, or HMAC-SHA256 with `--attest-key` (`env:VAR` or file; key never embedded). `attestation_id` is `uuid5` over host+timestamps so the whole body (and seal) is byte-reproducible under fixed inputs.
- **verify_attestation** re-derives records with the SAME extractor, recompares every record digest + the Merkle root + the body seal (`hmac.compare_digest`), and cross-checks body↔manifest record sets. **SECURITY (adversarial-review HIGH, fixed)**: when a key is supplied the caller REQUIRES a keyed seal — a keyless SHA-256 seal is rejected as a **downgrade** (else an attacker without the key could edit the body, recompute the public manifest/root, and swap in a keyless seal — the alg-confusion / alg:none attack). Never call any of this a "digital signature" (stdlib has no PKI) — it is an integrity digest / keyed integrity seal.
